...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (attackerHoldEffect == HOLD_EFFECT_THICK_CLUB && (attacker->species == SPECIES_CUBONE || attacker->species == SPECIES_MAROWAK))
        attack *= 2;
    if (attackerHoldEffect == HOLD_EFFECT_STICK  && (attacker->species == SPECIES_FARFETCHD || attacker->species == SPECIES_SUDOWOODO))
        attack *= 2;
    if (defenderHoldEffect == HOLD_EFFECT_UP_GRADE && defender->species == SPECIES_PORYGON)
        defense *= 2;
    if (defenderHoldEffect == HOLD_EFFECT_STEEL_POWER && defender->species == SPECIES_ONIX)
        spDefense *= 2;
    if (defenderHoldEffect == HOLD_EFFECT_STEEL_POWER && defender->species == SPECIES_SCYTHER)
        defense *= 2;
    if (defenderHoldEffect == HOLD_EFFECT_DRAGON_SCALE && defender->species == SPECIES_SEADRA)
        defense *= 2;
